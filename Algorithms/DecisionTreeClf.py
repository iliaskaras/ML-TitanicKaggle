...
class DecisionTreeClf(object):
    ...
    def createDecisionTreeClassifier(self, split_function, maxdepth):
        ...
        print(" ========= criterion: " + split_function + " ============= max depth: " + str(
            maxdepth) + " ==============")

    def getCrossValidation(self):
        if self.clf is not None:
            crossValResults = cross_val_score(self.clf, self.X_test, self.y_test)
    ...
    def getPrecisionScore(self):
        if self.clf is not None:
            crossValResults = cross_val_score(self.clf, self.X_test, self.y_test, cv=3, scoring="precision")
    ...
    def getRecallScore(self):
        if self.clf is not None:
            crossValResults = cross_val_score(self.clf, self.X_test, self.y_test, cv=3, scoring="recall")
            average = np.mean(crossValResults)
    ...
    def getF1Score(self):
        if self.clf is not None:
            crossValResults = cross_val_score(self.clf, self.X_test, self.y_test, cv=3, scoring="f1")
            average = np.mean(crossValResults)
    ...
    def getAccuracyScore(self):
        if self.clf is not None:
            crossValResults = cross_val_score(self.clf, self.X_test, self.y_test, cv=3, scoring="accuracy")
    ...
    def createDataframe(self,YtestUnlabeledPredictions):
        self.yTestPredictions_dataframe = pd.DataFrame(YtestUnlabeledPredictions, columns=['Survived'])
        yTestPassengerId_dataframe = pd.DataFrame(self.yTestPassengerId, columns=['PassengerId'])
        self.finalSubmissionDf = yTestPassengerId_dataframe.join(self.yTestPredictions_dataframe)
    ...
    def get_MethodDetailsMsg(self):
        return " =================================================================================\n" \
               " DecisionTreeClf with Split Function : " + self.split_function + ", Max Depth : " + str(
            self.max_depth) + "\n" \
                              " =================================================================================\n"
